...
CREATE PROCEDURE [PANINI_GDD].insertar_provincias
AS
BEGIN
	...
	SELECT DISTINCT CLIENTE_CODIGO_POSTAL,(SELECT CODIGO_PROVINCIA FROM PANINI_GDD.PROVINCIA p WHERE p.NOMBRE_PROV=CLIENTE_PROVINCIA) FROM gd_esquema.Maestra WHERE CLIENTE_CODIGO_POSTAL IS NOT NULL
	UNION ALL
	SELECT DISTINCT PROVEEDOR_CODIGO_POSTAL,(SELECT CODIGO_PROVINCIA FROM PANINI_GDD.PROVINCIA p WHERE p.NOMBRE_PROV=PROVEEDOR_PROVINCIA) FROM gd_esquema.Maestra WHERE PROVEEDOR_CODIGO_POSTAL IS NOT NULL

END
GO

CREATE PROCEDURE [PANINI_GDD].insertar_proveedor --hay q editar
AS
BEGIN
...
CREATE PROCEDURE [PANINI_GDD].insertar_productos --version vieja
AS
BEGIN
...
CREATE PROCEDURE [PANINI_GDD].insertar_productos_v2
AS
BEGIN
SELECT * FROM PANINI_GDD.CATEGORIA
	INSERT INTO PANINI_GDD.PRODUCTO (COD_PROD,NOMBRE_PROD,DESCRIPCION_PROD,ID_MARCA_PROD,ID_CATEGORIA_PROD,ID_MATERIAL_PROD)
	 SELECT DISTINCT PRODUCTO_CODIGO,PRODUCTO_NOMBRE,PRODUCTO_DESCRIPCION,
	 (SELECT ID_MARCA FROM PANINI_GDD.MARCA m WHERE m.MARCA=PRODUCTO_MARCA) ID_MARCA,
	 (SELECT ID_CATEGORIA FROM PANINI_GDD.CATEGORIA c WHERE c.CATEGORIA=PRODUCTO_CATEGORIA) ID_CATEGORIA_PROD,
	 (SELECT ID_MATERIAL FROM PANINI_GDD.MATERIAL ma WHERE ma.MATERIAL=PRODUCTO_MATERIAL) ID_MATERIAL_PROD
      FROM gd_esquema.Maestra
      WHERE PRODUCTO_CODIGO IS NOT NULL
END
GO
...
CREATE PROCEDURE [PANINI_GDD].insertar_variantes_v2 --SIN TERMINAR
AS
BEGIN
	INSERT INTO PANINI_GDD.TIPO_VARIANTE (CODIGO_TIPO_VAR,DESCRIPCION_TIPO_VAR)
	SELECT DISTINCT PRODUCTO_VARIANTE_CODIGO,PRODUCTO_TIPO_VARIANTE FROM gd_esquema.Maestra WHERE PRODUCTO_VARIANTE_CODIGO IS NOT NULL

	INSERT INTO PANINI_GDD.VARIANTE (CODIGO_VAR,CODIGO_TIPO_VAR,VALOR)
...
BEGIN
  ...
  BEGIN
		...
		BEGIN
			...
			SELECT DISTINCT VENTA_CODIGO,VENTA_CUPON_CODIGO,VENTA_CUPON_IMPORTE FROM gd_esquema.Maestra
		END
		FETCH NEXT FROM cursorvs INTO @codVenta, @fecha, @clienteDni, @canalVenta, @medioEnvio, @medioPago, @totalVenta, @precioEnvio,@cuponCod,
		@cuponImporte,@descuentoConcepto,@descuentoValor,@codPostal,@envioPrecio,@nombre,@apellido,@canalCosto
  	END
...
CREATE PROCEDURE [PANINI_GDD].insertar_ventas_v2
AS
BEGIN
	INSERT INTO PANINI_GDD.DESCUENTO_CUPON (CODIGO_CUPON,FECHA_DESDE,FECHA_HASTA,VALOR_DESC)
	SELECT DISTINCT VENTA_CUPON_CODIGO,VENTA_CUPON_FECHA_DESDE,VENTA_CUPON_FECHA_HASTA,VENTA_CUPON_VALOR FROM gd_esquema.Maestra WHERE VENTA_CUPON_CODIGO IS NOT NULL
	

	INSERT INTO PANINI_GDD.VENTA (COD_VENTA, FECHA_VENTA, ID_CLIENTE , ID_CANAL_VENTA ,ID_MEDIO_ENVIO, 
	ID_MEDIO_PAGO, TOTAL_VENTA, PRECIO_ENVIO, CANAL_COSTO, COSTO_TRANSACCION)
	
	SELECT DISTINCT VENTA_CODIGO,
	VENTA_FECHA, 
	(SELECT ID_CLIENTE FROM PANINI_GDD.CLIENTE c WHERE c.DNI_CLIENTE=CLIENTE_DNI AND CLIENTE_APELLIDO=c.APELLIDO_CLIENTE) ID_CLIENTE, 
	(SELECT ID_CANAL_VENTA FROM PANINI_GDD.CANAL_VENTA cv WHERE cv.CANAL_VENTA=VENTA_CANAL) ID_CANAL_VENTA, 
	(SELECT ID_MEDIO_ENVIO FROM PANINI_GDD.MEDIO_ENVIO_X_CODIGO_POSTAL m WHERE m.MEDIO=VENTA_MEDIO_ENVIO AND m.CODIGO_POSTAL=CLIENTE_CODIGO_POSTAL AND VENTA_ENVIO_PRECIO=m.PRECIO) ID_MEDIO_ENVIO,
	(SELECT ID_MEDIO_PAGO FROM PANINI_GDD.MEDIO_DE_PAGO p WHERE p.MEDIO_PAGO=VENTA_MEDIO_PAGO AND p.VALOR_DESC=VENTA_DESCUENTO_IMPORTE AND  VENTA_DESCUENTO_CONCEPTO!='Otros' AND VENTA_CUPON_VALOR IS NULL) ID_MEDIO_PAGO,
	VENTA_TOTAL, 	
	VENTA_CANAL_COSTO,
	VENTA_MEDIO_PAGO_COSTO
	FROM gd_esquema.Maestra WHERE VENTA_CODIGO IS NOT NULL AND (SELECT ID_MEDIO_PAGO FROM PANINI_GDD.MEDIO_DE_PAGO p WHERE p.MEDIO_PAGO=VENTA_MEDIO_PAGO AND p.VALOR_DESC=VENTA_DESCUENTO_IMPORTE AND VENTA_DESCUENTO_CONCEPTO!='Otros') IS NOT NULL
	...
	(SELECT ID_CANAL_VENTA FROM PANINI_GDD.CANAL_VENTA cv WHERE cv.CANAL_VENTA=VENTA_CANAL) ID_CANAL_VENTA, 
	(SELECT ID_MEDIO_ENVIO FROM PANINI_GDD.MEDIO_ENVIO_X_CODIGO_POSTAL m WHERE m.MEDIO=VENTA_MEDIO_ENVIO AND m.CODIGO_POSTAL=CLIENTE_CODIGO_POSTAL AND VENTA_ENVIO_PRECIO=m.PRECIO) ID_MEDIO_ENVIO,
	(SELECT ID_MEDIO_PAGO FROM PANINI_GDD.MEDIO_DE_PAGO p WHERE p.MEDIO_PAGO=VENTA_MEDIO_PAGO AND p.VALOR_DESC=0 AND VENTA_DESCUENTO_CONCEPTO='Otros') ID_MEDIO_PAGO,
	VENTA_TOTAL, 	
	VENTA_CANAL_COSTO,
	VENTA_MEDIO_PAGO_COSTO
	FROM gd_esquema.Maestra WHERE VENTA_CODIGO IS NOT NULL AND (SELECT ID_MEDIO_PAGO FROM PANINI_GDD.MEDIO_DE_PAGO p WHERE p.MEDIO_PAGO=VENTA_MEDIO_PAGO AND p.VALOR_DESC=0 AND VENTA_DESCUENTO_CONCEPTO='Otros') IS NOT NULL
	
	SELECT DISTINCT VENTA_CODIGO,ID_MEDIO_PAGO FROM gd_esquema.Maestra ma
	JOIN PANINI_GDD.MEDIO_DE_PAGO m ON ma.VENTA_MEDIO_PAGO=m.MEDIO_PAGO AND ma.VENTA_DESCUENTO_IMPORTE=m.VALOR_DESC
	UNION
	SELECT DISTINCT VENTA_CODIGO,ID_MEDIO_PAGO FROM gd_esquema.Maestra ma
	JOIN PANINI_GDD.MEDIO_DE_PAGO m ON ma.VENTA_MEDIO_PAGO=m.MEDIO_PAGO AND ma.VENTA_DESCUENTO_IMPORTE IS NULL
	WHERE VENTA_CODIGO IS NOT NULL AND (SELECT ID_MEDIO_PAGO FROM PANINI_GDD.MEDIO_DE_PAGO p WHERE p.MEDIO_PAGO=VENTA_MEDIO_PAGO AND p.VALOR_DESC=0 AND VENTA_DESCUENTO_CONCEPTO='Otros')
...
select top 100 * from gd_esquema.Maestra where VENTA_DESCUENTO_CONCEPTO is not null
...
CREATE FUNCTION [PANINI_GDD].obtener_id_medio_pago(@codVenta decimal(19,0)) RETURNS DECIMAL(19,0) AS
  BEGIN
	DECLARE @medio nvarchar(255),@importe decimal(18,2)
    SELECT distinct @medio=VENTA_MEDIO_PAGO,@importe=VENTA_DESCUENTO_IMPORTE 
	FROM PANINI_GDD.Maestra 
	...
	IF @importe IS NULL
	RETURN (SELECT ID_MEDIO_PAGO FROM [PANINI_GDD].MEDIO_DE_PAGO WHERE MEDIO_PAGO=@medio AND VALOR_DESC=0)
	ELSE
	RETURN (SELECT ID_MEDIO_PAGO FROM [PANINI_GDD].MEDIO_DE_PAGO WHERE MEDIO_PAGO=@medio AND VALOR_DESC=@importe)
...
create procedure [PANINI_GDD].insertar_compras_3
AS
BEGIN
	SELECT DISTINCT 
			...
			maestra.VENTA_FECHA,
			cliente.ID_CLIENTE,
			canalDeVenta.ID_CANAL_VENTA,
			medioDeEnvio.ID_MEDIO_ENVIO,
			(
			SELECT TOP 1 ID_MEDIO_PAGO FROM [PANINI_GDD].[MEDIO_DE_PAGO] medioDePago
			WHERE (maestra.VENTA_MEDIO_PAGO = medioDePago.[MEDIO_PAGO] AND maestra.VENTA_DESCUENTO_IMPORTE = medioDePago.VALOR_DESC and maestra.VENTA_DESCUENTO_IMPORTE !='Otros') OR (0 = medioDePago.VALOR_DESC AND VENTA_DESCUENTO_CONCEPTO='Otros')
			) ID_MEDIO_PAGO,
			maestra.VENTA_TOTAL
		...
		ON		maestra.CLIENTE_NOMBRE		= cliente.NOMBRE_CLIENTE
			AND maestra.CLIENTE_APELLIDO		= cliente.APELLIDO_CLIENTE
			AND maestra.CLIENTE_DNI			= cliente.DNI_CLIENTE
			AND maestra.CLIENTE_DIRECCION	= cliente.DIRECCION_CLIENTE
			--AND maestra.CLIENTE_TELEFONO		= cliente.TELEFONO_CLIENTE
			--AND maestra.CLIENTE_MAIL			= cliente.MAIL_CLIENTE
			--AND maestra.CLIENTE_FECHA_NAC	= cliente.FECHA_NAC_CLIENTE
			--AND maestra.CLIENTE_LOCALIDAD	= cliente.LOCALIDAD_CLIENTE
			--AND codigoPostal.CODIGO_POSTAL = cliente.CODIGO_POSTAL
			--AND maestra.CLIENTE_CODIGO_POSTAL= cliente.CLIENTE_CP
			--AND maestra.CLIENTE_PROVINCIA	= cliente.CLIENTE_PROVINCIA
		WHERE (
			SELECT TOP 1 ID_MEDIO_PAGO FROM [PANINI_GDD].[MEDIO_DE_PAGO] medioDePago
			WHERE maestra.VENTA_MEDIO_PAGO = medioDePago.[MEDIO_PAGO] AND maestra.VENTA_DESCUENTO_IMPORTE = medioDePago.VALOR_DESC OR (0 = medioDePago.VALOR_DESC AND VENTA_DESCUENTO_CONCEPTO='Otros')
			) IS NOT NULL
		order by venta_codigo
